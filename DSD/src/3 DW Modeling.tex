...
This section will introduce three models to define the structure of a data warehouse: for the conceptual level, the \textbf{Dimensional Fact Model} (\textbf{DFM}), while for the logical level, the \textbf{Multidimensional Relational Model} and the \textbf{Multidimensional Cube Model} (which will be discussed in a later chapter).
...
Hierarchies may be \textbf{recursive}, when it involves the same dimension. For example, an \textit{employee} may have an assigned role that puts them in charge of other employees, as well as being managed by someone else; this relationship can be expressed via the \textit{supervisor} attribute, which allows the creation of a hierarchy of employees in a manager-subordinate relationship. Recursivity is represented by a crossed arrow that loops back in the same dimension. Hierarchies can also be \textbf{ragged}, if one or more elements in the chain are optional. For example, an hierarchy can be built with the attributes \textit{City}, \textit{State}, and \textit{Country}. Since some cities may belong to countries without states, \textit{State} can be set as optional. Graphically, the optional attributes in a ragged hierarchy are crossed by a line. Attributes and hierarchies can be \textbf{shared} among multiple dimensions. These attributes are represented by a double circle, and their incoming arc are always directed (regardless of whether they belong to a hierarchy or not).

A dimension may also be \textbf{multivalued}, if the same fact involves multiple instances of the same dimension. For example, a product order is managed by two agents, or a medical treatment involves multiple doctors. Multivalued dimensions are represented by a double arrow head exiting from the fact table. A dimension without attributes is called \textbf{degenerate}. They are simply represented by a line exiting the fact table and a singular circle at the end without any additional attributes. If a dimension is \textbf{optional}, it is represented by a dashed line.
...
\paragraph{Constellation Schema} A constellation schema is obtained by fusing multiple schemas with one or more dimensions in common, called \textbf{conformed dimensions}. The result is a schema with several fact tables that share a number of dimension tables.
\begin{figure}[ht]
    \centering
